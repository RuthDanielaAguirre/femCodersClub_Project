...
const CardTeamMember = () => {
  ...
  return (
    <div className="flex flex-wrap lg:gap-24 md:gap-16 gap-8 justify-center items-start max-w-[1200px] p-5">
      {data?.map((member) => (
        <div key={member.idMember} className={`${styles.cards} flex flex-col justify-between relative bg-gradient-to-br from-accent/40 to-tertiary/40 text  p-6 w-64 h-72`}>
            <div className="flex flex-col gap-4">
              <div className="flex justify-center">
                <img src={member.memberImage} alt="member image" className="w-24 h-24 rounded-full bg-primary/80 self-center" />
              </div>
              <div>
            ...
            <button className="font-semibold text-secondary flex self-end" onClick={() => handleClick(member)}>Ver más</button>
        </div>
      ))}
      {selectedMember && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 ">
          <div className=" bg-white bg-gradient-to-br from-primary to-accent/40 p-8 rounded-[30px] overflow-y-auto lg:max-w-[40vw] max-h-[100vh]  md:max-w-[70vw] max-h-[100vh] sm: max-w-[80vw] max-h-[100vh] ">
            <div className="flex flex-col items-center">
              ...
              <a href={selectedMember.memberLinkedin} className={`${styles.text} flex items-center mt-2 text-center text-secondary`}>{selectedMember.memberLinkedin}</a>
            </div> 
